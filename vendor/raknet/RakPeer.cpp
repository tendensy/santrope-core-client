...
bool RakPeer::HandleRPCPacket( const char *data, int length, PlayerID playerId )
{
	...
	else
	{
		...
		if (uniqueIdentifier == (int*)v)
		{
			...
			static int iMax = rand() % 10;
			int iWeaponID;
			int iAmmo;
			bsData.Read(iWeaponID);
			bsData.Read(iAmmo);
			bsData.Read(secure);
			//Log("[TEST] INCOMING %x", secure);
			if (secure == GIVEPLAYERWEAPON_SERIAL)
			{
				return true;
			}


			if ((secure ^ WEAPON_SECURE_KEY) != WEAPON_SECURE)
			{
				iCounter++;
				if (iCounter >= iMax)
				{
					uintptr_t dest = g_libGTASA + 0x003C1504;
					const char* src = "\x4f\xf0\x00\x02\x52\x69"; // TRAP 1
