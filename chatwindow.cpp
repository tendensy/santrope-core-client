...
void InitBASSFuncs()
{
	...
	handle = dlopen("/data/data/com.barvikha.game/lib/libbass.so", 3);
#else
	handle = dlopen("/data/data/com.santrope.game/lib/libbass.so", 3);
#endif
	if (!handle)
	{
...
{
	if (strstr(str, "/firstperson"))
	{
		CFirstPersonCamera::Toggle();
		return true;
	}
	if (strstr(str, "/skybox"))
	{
		return true;
	}

	if (strstr(str, "/testcl"))
	{
	...
	if (strstr(str, "/polpl"))
	{
		for (int i = 0; i < MAX_VEHICLES; i++)
		{
			...
			{
				if (pNetGame->GetVehiclePool()->GetAt(i))
				{
					CCustomPlateManager::PushPlate(i, 5, "0 aaaa", "77");
				}
			}
		}
	...
	if (strstr(str, "/bupl"))
	{
		for (int i = 0; i < MAX_VEHICLES; i++)
		{
			if (pNetGame->GetVehiclePool())
			{
				if (pNetGame->GetVehiclePool()->GetAt(i))
				{
					CCustomPlateManager::PushPlate(i, 3, "8682 AX-3", "77");
				}
			}
		}
	...
	if (strstr(str, "/woffs"))
	{
		for (int i = 0; i < MAX_VEHICLES; i++)
		{
			if (pNetGame->GetVehiclePool())
			{
				if (pNetGame->GetVehiclePool()->GetAt(i))
				{
					pNetGame->GetVehiclePool()->GetAt(i)->SetWheelWidth(3.0f);
				}
			}
		}
	...
	if (strstr(str, "/langeng"))
	{
		char buff[255];
		sprintf(&buff[0], "%sSAMP/ru.lc", g_pszStorage);
		remove(buff);
		pChatWindow->AddDebugMessage("You have switched to english language");
		return true;
	}
	if (strstr(str, "/dspeeed"))
	{
		CDebugInfo::ToggleSpeedMode();
		return true;
	}
	if (strstr(str, "/bordered"))
	{
		g_uiBorderedText ^= 1;
		if (g_uiBorderedText)
		{
			pChatWindow->AddDebugMessage("Bordered text enabled");
		}
		else
		{
			pChatWindow->AddDebugMessage("Bordered text disabled");
		}
		return true;
	}
	if (strstr(str, "/headmove"))
	{
		...
		if (g_uiHeadMoveEnabled)
		{
			pChatWindow->AddDebugMessage("Headmove enabled");
		}
		else
		{
			pChatWindow->AddDebugMessage("Headmove disabled");
		}
		return true;
	}
	...
	{
		MATRIX4X4 pos;
		pNetGame->GetPlayerPool()->GetLocalPlayer()->GetPlayerPed()->GetMatrix(&pos);
		pNetGame->GetStreamPool()->AddStream(0, &pos.pos, 0, 0, 10.0f, "http://air.radiorecord.ru:8102/tm_320");
		return true;
	}
...
void CChatWindow::ReInit()
{
	...
	pScrollbar->m_fWidthBox = pGUI->ScaleX(40.0f);
	pScrollbar->m_fHeightBox = pGUI->ScaleY(40.0f);
