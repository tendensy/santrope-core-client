...
RwTexture* CCustomPlateManager::ProcessRuPlate(SCustomPlate& plate)
{
	...
	pText->Draw(22.0f, 12.0f, 160.0f, 44.0f, white);
	pRegion->Draw(198.0f, 14.0f, 38.0f, 24.0f, white);
	RwTexture* pTexture = m_pRenderTarget->End();
...
void CCustomPlateManager::Initialise()
{
	...
	m_pUaSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB("gta_int", "plate_ua");
	m_pRuSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB("gta_int", "plate_ru");
	m_pBuSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB("gta_int", "plate_bu");
	m_pKzSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB("gta_int", "plate_kz");
	m_pRuPoliceSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB("gta_int", "plate_ru_police");
	...
	for (auto& i : m_Plates)
	{
		...
		if (pPlate)
		{
			if (pNetGame->GetVehiclePool())
			{
				...
				{
					pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture("plate_number", pPlate);
				}
			}
		}
	}
