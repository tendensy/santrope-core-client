...
void CSettings::Save(int iIgnoreCategory)
{
	...
	ini_table_create_entry_as_int(config, "gui", "iVoiceList", m_Settings.iVoiceList);
	...
	if (iIgnoreCategory != 1)
	{
		...
		ini_table_create_entry_as_int(config, "gui", "outfitguns", m_Settings.iOutfitGuns);
	...
	if (iIgnoreCategory != 2)
	{
		...
		{
			char buff[30];
			snprintf(buff, sizeof(buff), "element_color_%d", i);
...
void CSettings::LoadSettings(const char* szNickName, int iChatLines)
{
	...
	m_Settings.iVoiceList = ini_table_get_entry_as_int(config, "gui", "iVoiceList", 1);
	...
	m_Settings.iOutfitGuns = ini_table_get_entry_as_int(config, "gui", "outfitguns", 1);
