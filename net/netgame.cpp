...
void CNetGame::Packet_TrailerSync(Packet* p)
{
	...
	bsSpectatorSync.Read(trSync.trailerID);
	if ((bsSpectatorSync.GetNumberOfUnreadBits() / 8))
	{
		VEHICLEID TrailerID = trSync.trailerID;
		if (!TrailerID) return;

		CVehicle* pVehicle = GetVehiclePool()->GetAt(TrailerID);
		bsSpectatorSync.Read((char*)pVehicle, sizeof(TRAILER_SYNC_DATA) - sizeof(trSync.trailerID));
	}
		...
		case RPC_CUSTOM_VISUALS:
		{
			...
			if (GetVehiclePool())
			{
				...
				if (pVeh)
				{
					...
					pVeh->SetStrobs(bStrobs, bStrobsPolice);
					if (fWheelWidth)
					{
